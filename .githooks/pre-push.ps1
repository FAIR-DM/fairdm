...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host ""
    Write-Host "Tip: Run 'uv run invoke format' to auto-fix formatting issues" -ForegroundColor Yellow
    Write-Host "Or run 'uv run invoke pre-push' to see all issues" -ForegroundColor Yellow
    Write-Host ""
    exit 1
}
