...
def _media_root_under_tmp_path(tmp_path, settings):
    ...
    ``tests/settings.py`` falls back to a per-process temp path for code that
    reads these settings outside a test, but every test overrides it with this
    before its body runs. A model instance saved with a real image writes it
    under ``MEDIA_ROOT``, so a fixed shared path here meant every run of the
    suite wrote into the same directory and nothing ever removed it
    (issue #323).
