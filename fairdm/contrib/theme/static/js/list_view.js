...
function initializeSearchFunctionality() {
  const hiddenInput = document.getElementById('search-hidden')
  const searchFields = document.querySelectorAll('.search-field')

  if (hiddenInput && searchFields.length > 0) {
    let searchTimeout

    // Update all visible fields when hidden field has a value on page load
    const initialValue = hiddenInput.value || ''
    searchFields.forEach(field => {
      field.value = initialValue
    })

    // Synchronize visible fields with hidden input
    searchFields.forEach(field => {
...
function initializeSortFunctionality() {
  ...
  const hiddenInput = document.getElementById('ordering-hidden')

  // Initialize visible selects with the hidden input value on page load
  if (hiddenInput && sortSelects.length > 0) {
    const initialValue = hiddenInput.value || '-created'
  ...
  sortSelects.forEach(select => {
    select.addEventListener('change', function () {
      ...
      // Submit the filter form instead of directly manipulating URL
