...
class Overview(CRUDDirectoryMixin, OverviewPlugin):
    ...
    and "Delete" buttons), and ``crud_views`` reverses each to :class:`Update`'s and
...
class Descriptions(Plugin, MetadataMixin, MVPFormView):
    ...
    plan P2) — unlike :class:`Update` and :class:`Delete`, whose owner is :class:`Overview`.
    ...
    # template from a model the way Update's own view does, so leaving template_name unset
