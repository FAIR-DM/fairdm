...
| G9 | Records in the core model can be searched, sorted and filtered from the portal | Essential | | |
| G10 | Data and metadata are reachable by machines through a documented API | Expected | | |
| G11 | Round-trip import and export of tabular data | Expected | | |
| G12 | Private and public data sit side by side, controlled per object | Expected | | |
| G13 | A dataset moves from working state to visible through a checked process | Expected | | |
| G14 | Dataset metadata is complete enough for a formal-publication addon to submit it unaided | Expected | | |
| G15 | External identifiers for people, organisations and samples are carried through the record | Expected | | |
| G16 | A research group with no operations staff can deploy and run its own portal | Expected | | |
| G17 | A domain schema built by one community installs as a package in another portal | Expected | | |
| G18 | The framework grows through addons while the core stays small | Expected | | |
| G19 | A dataset carries versions, so an earlier state of it stays retrievable and citable | Expected | | |
| G20 | Portals exchange data with one another | Aspirational | | |
| G21 | A portal supports the research community around it, not only its data | Aspirational | | |
| G22 | Contributors use a portal in their own language and regional conventions | Aspirational | | |
