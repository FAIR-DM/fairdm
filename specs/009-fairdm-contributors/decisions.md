...
## D25 — `username = None`, accepting the fallback-display side effect it exposes
...
## D26 — Email uniqueness moves from a field flag to a `Meta.constraint`, not alongside it
...
## D27 — `UserManager` keeps its name; `create_user` is the only addition
