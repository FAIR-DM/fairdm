...
## D27 — Primary-membership atomicity is tested by patching the base `Model.save`
...
## D28 — T102's receiver is additive, not a replacement for the lifecycle hook
...
## D29 — `Contributor.samples`/`.measurements` fixed for polymorphic content types, in scope
...
## D30 — `fairdm/core/abstract.py`'s `add_contributor` keeps the same `roles.set()` defect, left alone
...
## D31 — `test_contribution_multiple_roles` edited under the story brief's explicit direction
...
after all.
## D40 — T005's fixture is added despite the substance already existing
...
## D41 — T001's `default_auto_field` declaration is cosmetic, not functional
