...
`ProjectConfigure` (`fairdm/core/project/plugins.py:40-48`) is a registered plugin with the label
"Configure", the icon `settings` and the route `project/<uuid>/project-configure/`. It is a
`FairDMUpdateView` over `["name", "visibility", "owner"]`. Confirmed registered by listing the
registry:
...
no standard Django backend can grant. Only the object-level layer can.

This corrects my earlier report on the specification. I said there were no edit or delete links
anywhere. There is an edit route: the Configure tab. What is true is narrower and cheaper to fix
than I implied, as the next section shows.
...
- `directory` — the list of actions resolved into context. `get_directory()` loops over this and
  ...
  (`detail.py:212`).
- `show_<action>_action` — `False` by default (`detail.py:53-57`). It accepts a bool, or a method
  taking the user. It must be a method and not a lambda assigned in the class body: the flag is read
  with `getattr`, so a plain function binds as a method and is then called with one argument too
  many. Confirmed on this branch.
...
list, and a view lists them on `inlines`. The formsets arrive in context as `inlines`
...
attaches to the `value` field. Two conditions apply. `value` must be in the inline's `fields`, or
...
- `contributors/add/` returns 500 to an anonymous request, not 403.
  ...
  does not accept. The missing permission check and the crash are separate faults. The crash is what
