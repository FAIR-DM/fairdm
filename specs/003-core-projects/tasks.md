...
- [x] T001 A factory for project descriptions, producing a valid type and body.
...
- [x] T004 Test: attaching a second description of a type the project already carries fails
      validation, and the message names the type.
- [x] T005 Validation refusing a duplicate description type.
- [x] T006 Test: the duplicate is refused at the database as well, so a concurrent write cannot slip
      past validation.
- [x] T007 A database constraint making description type unique per project.
...
- [x] T008 Test: a term from a configured controlled vocabulary added as a keyword is stored as a
      reference to that vocabulary rather than as text.
- [x] T009 Test: free tags are stored and remain distinguishable from controlled keywords.
- [x] T010 Keyword and tag fields on the project.

## US-2 — Project dates and the timeline check (#159)

- [x] T011 A factory for project dates, defaulting to a type the project date vocabulary contains.
...
- [x] T014 Test: a second start date on the same project is refused.
- [x] T015 A database constraint making date type unique per project.
...
- [x] T016 Test: an end date earlier than the project's start is refused, with a message naming both
      dates.
- [x] T017 Test: changing the start to a date after the existing end is refused for the same reason.
- [x] T018 Test: an end date on a project with no start date is accepted.
- [x] T019 Test: a year-only end date in the same year as a month-precision start is accepted —
      comparison happens at the coarser precision.
- [x] T020 Cross-record validation of the project timeline, comparing at the coarser of the two
      precisions.

## US-3 — Project identifiers (#160)

- [x] T021 A project collection in the identifier vocabulary, containing a DOI, a grant number and a
      proposal identifier.
- [x] T022 A registry key for the identifier vocabulary that does not collide with another
...
- [x] T023 The project identifier record bound to the project collection rather than to the whole
      vocabulary.
- [x] T024 A factory for project identifiers.
- [x] T025 Test: a DOI attached to a project is stored under the DOI type.
- [x] T026 Test: a grant number is stored alongside the DOI.
- [x] T027 Test: attaching the same identifier value to a second project is refused.
- [x] T028 Test: the identifier types offered for a project contain a DOI and a grant number, and
      contain no identifier that names a person or an organisation.

## US-4 — Funding in DataCite's shape (#161)

- [x] T029 A validator for funding, accepting a list of DataCite funding references. The accepted
...
- [x] T030 The funding field carrying that validator.
- [x] T031 Test: an award with funder name, funder identifier, scheme, award number and award title
      round-trips with each part readable individually.
- [x] T032 Test: a project carrying two awards retains both.
- [x] T033 Test: an award naming only a funder is accepted.
- [x] T034 Test: an award naming a funder identifier scheme outside DataCite's set is refused, and
      the message names the accepted schemes.
- [x] T035 Test: funding that is not a list is refused, and a list whose members are not objects is
      refused with the same message rather than raising.
- [x] T036 The project factory emits funding in the specified shape.
- [x] T037 A reversible migration converting stored funding from the older flat shape to the list of
      funding references.
- [x] T038 The pre-existing factory tests asserting the flat shape are rewritten against the
      specified one.

## US-5 — Metadata export (#162)

- [x] T039 A module mapping a project to DataCite's JSON form, carrying its descriptions, dates,
      identifiers, contributions and funding.
- [x] T040 Test: exporting a project that carries every kind of related record produces output
      containing every one of them.
- [x] T041 Test: where a project carries a DOI, the export presents it as the record's primary
      identifier.
- [x] T042 Test: exporting a minimally populated project omits the absent parts rather than emitting
      empty structures.
- [x] T043 A module mapping a project to JSON-LD with an explicit context. The contributor block
...
- [x] T044 Test: the JSON-LD output parses as JSON-LD and carries a context.
- [x] T045 Administrative actions serialising both forms over a selection.
- [x] T046 Test: exporting several projects together produces output carrying all of them.

## US-6 — The administrative interface (#163)

- [x] T047 Administrative search over project name, the project's own generated identifier, any
      ...
      identifier is not.
- [x] T048 Test: a search term matching each of those three finds the project.
...
- [x] T050 Test: the status filter leaves only projects with that status.
      **Done:** fairdm/core/project/admin.py:57 — covered by TestAdminFilterByStatus::test_filter_by_concept_status
- [x] T051 Inline editing of a project's descriptions, dates and identifiers from its own page.
- [x] T052 Test: a description, a date and an identifier added inline all persist.
- [x] T053 List columns showing whether a project carries an abstract and whether it carries a start
      date.
- [x] T054 Test: those columns reflect the presence and absence of each.
- [x] T055 Bulk actions setting a project's status.
- [x] T056 Test: every bulk status action leaves the selected projects in the status its label names.

## US-7 — The creation record (#164)

- [x] T057 A nullable reference from a project to the user who created it, surviving that user's
      removal. Not editable, and carrying `verbose_name` and `help_text` per Article IX.
- [x] T058 A migration adding that reference.
- [x] T059 The creator written wherever a project is created: the portal create view, and an
      ...
      admin field.
- [x] T060 Test: a project created by a known user names that user as its creator.
- [x] T061 Test: a project whose creator's account has been removed survives with its creator reading
      as unknown.
- [x] T062 Test: changing a project advances its modification timestamp and leaves its creator
      unchanged.
...
- [x] T065 A required name, and an optional image, owning organisation and funding.
- [x] T066 Test: a project with no owning organisation is valid.
- [x] T067 A lifecycle status drawn from a controlled set, every member labelled with the state it
      names. The fields this task touches carry `verbose_name` and `help_text` per Article IX.
- [x] T068 Test: every status label matches the state its member names.
...
- [x] T071 Projects ordered most-recently-modified first by default.
- [x] T072 Test: the default ordering is most-recently-modified first.
...
- [x] T075 Test: every role in the project role vocabulary is expressible as a DataCite contributor
      type.
- [x] T076 Every string the model, its vocabularies, its validation messages and its administrative
      interface present to a user marked for translation in a way that resolves at request time.
- [x] T077 Test: none of those surfaces binds a translated string at import time.
- [x] T078 A queryset loading a project together with its descriptions, dates, identifiers,
      contributions and keywords in a bounded number of queries.
- [x] T079 Test: the query count for loading a project with all its related metadata does not grow
...
All seventy-nine tasks are closed. The eight that were proven done at reconciliation were left
alone; the remaining seventy-one were built or tested on this branch.

The project test module went from 77 tests to 128, with the two skipped date tests now passing and
the vacuous bulk-status test replaced by one that asserts what the action leaves behind.
