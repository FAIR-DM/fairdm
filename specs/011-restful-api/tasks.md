...
- [x] T005 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check` â€” MUST pass before proceeding to Phase 2

**Checkpoint â€” Setup Complete**: Package exists, all dependencies installed, apps registered, system checks pass.

---

## Phase 2: Foundational (Blocking Prerequisites)

**Purpose**: Create all base classes and configuration that every user story depends on. No API endpoints are exposed yet â€” this phase builds the building blocks.

**âš ï¸ CRITICAL**: No user story work can begin until this phase is complete.

- [x] T006 Create `fairdm/api/settings.py` with `REST_FRAMEWORK` dict (authentication classes, permission classes, renderer classes with ORJSONRenderer, parser classes with ORJSONParser, schema class, pagination class, throttle classes/rates, filter backends per data-model.md), `SPECTACULAR_SETTINGS` (title, description, version, sidecar config), and `CORS_*` defaults â€” integrate into Django settings via `fairdm/conf/settings/` or `fairdm.setup()`
...
- [x] T008 [P] Create `fairdm/api/filters.py` with `FairDMVisibilityFilter(BaseFilterBackend)`: `filter_queryset()` returns `queryset.filter(is_public=True) | get_objects_for_user(user, view_perm, queryset)` for authenticated users, `queryset.filter(is_public=True)` for anonymous â€” per data-model.md pseudocode; add `hasattr(queryset.model, 'is_public')` guard at the top of `filter_queryset()` to short-circuit models without `is_public` (e.g., Contributor) and return the full unfiltered queryset
...
- [x] T010 [P] Create `fairdm/api/serializers.py` with `BaseSerializerMixin` and serializer generation logic supporting three-tier resolution: `fields` â†’ `serializer_fields` â†’ `serializer_class` from registry config; integrate `ObjectPermissionsAssignmentMixin` from `djangorestframework-guardian` for permission assignment on create/update
- [x] T011 Create `fairdm/api/viewsets.py` with `BaseViewSet(ModelViewSet)`: `lookup_field="uuid"`, `get_queryset()` returning model's default queryset, `get_serializer_class()` resolving from registry config, `filterset_class` attachment from registry; also define explicit `ProjectViewSet(BaseViewSet)`, `DatasetViewSet(BaseViewSet)` (full CRUD; model-specific querysets), and `ContributorViewSet(ReadOnlyModelViewSet)` (GET-only; `lookup_field="uuid"`) â€” these three core viewset classes are registered by the router in T017 and must exist before T017 runs
...
### System Validation â€” Phase 2

- [x] T013 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check` â€” MUST pass before proceeding
- [x] T014 âš ï¸ CRITICAL: Run foundational tests: `poetry run pytest tests/test_api/conftest.py --collect-only -v` â€” ALL fixtures must be importable and collectible before proceeding to any user story

**Checkpoint â€” Foundation Ready**: All base classes importable, settings merged, test fixtures available. User story phases can now begin.

---

## Phase 3: User Story 1 â€” Read-Only Browsing of Core Data via API (Priority: P1) ðŸŽ¯ MVP
...
**Independent Test**: Start the portal with demo app models registered, make unauthenticated GET requests to projects list, datasets list, sample discovery, and type-specific sample list endpoints â€” verify paginated JSON responses with correct fields and navigation links.
...
- [x] T016 [US1] Add `SampleDiscoveryView(APIView)` and `MeasurementDiscoveryView(APIView)` to `fairdm/api/viewsets.py` â€” GET-only views that iterate over `registry.samples` / `registry.measurements` and return the type catalog JSON (name, verbose_name, endpoint URL, fields, filterable_fields, count) per data-model.md and contract Â§6
...
- [x] ⚠️ Reopened T018 [US1] (reopened â€” BUG-001) Create `fairdm/api/urls.py` with `/api/v1/` URL patterns and explicit URL namespace isolation (`app_name = "api"`) so API route names do not collide with portal UI routes; include `fairdm_api_router.urls`, wire `SampleDiscoveryView` at `samples/`, wire `MeasurementDiscoveryView` at `measurements/`, add `dj-rest-auth` URLs at `auth/`, add `SpectacularAPIView` at `schema/`, `SpectacularSwaggerView` at `docs/`, `SpectacularRedocView` at `redoc/`
- [x] ⚠️ Reopened T019 [US1] (reopened â€” BUG-001) Include API URL patterns in `fairdm/conf/urls.py` using namespace isolation (e.g., `include(("fairdm.api.urls", "api"), namespace="api")`) to prevent URL name collisions with server-rendered portal views
...
- [x] T023 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check` â€” MUST pass before proceeding
- [x] T024 âš ï¸ CRITICAL: Run User Story 1 tests: `poetry run pytest tests/test_api/test_viewsets.py tests/test_api/test_router.py tests/test_api/test_pagination.py -v` â€” ALL tests MUST pass

**Checkpoint â€” US1 Complete**: All list/detail/discovery endpoints return correct JSON. Pagination works. The API is browseable at `/api/v1/`. This is the MVP â€” deployable as a read-only API.

---

## Phase 4: User Story 2 â€” Interactive API Documentation (Priority: P2)
...
- [x] T027 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check` â€” MUST pass before proceeding
- [x] T028 âš ï¸ CRITICAL: Run User Story 2 tests: `poetry run pytest tests/test_api/test_docs.py -v` â€” ALL tests MUST pass

**Checkpoint â€” US2 Complete**: Interactive documentation accessible, accurate, and reflecting all registered endpoints.

---

## Phase 5: User Story 3 â€” Authenticated CRUD Operations (Priority: P3)
...
- [x] T029 [US3] Ensure `BaseViewSet` in `fairdm/api/viewsets.py` enforces authentication for write operations: `perform_create()` checks add permission, `perform_update()` checks change permission, `perform_destroy()` checks delete permission â€” unauthenticated write attempts return 401 per contract Â§3/Â§4/Â§5
...
- [x] T033 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check` â€” MUST pass before proceeding
- [x] T034 âš ï¸ CRITICAL: Run User Story 3 tests: `poetry run pytest tests/test_api/test_auth.py tests/test_api/test_viewsets.py -v` â€” ALL tests MUST pass

**Checkpoint â€” US3 Complete**: Full CRUD lifecycle works through the API. Authentication endpoints functional. Permission assignment on create/update operational.

---

## Phase 6: User Story 4 â€” Permission-Enforced Access Control (Priority: P4)
...
**Independent Test**: Create a private dataset. GET as anonymous â†’ 404. GET as unrelated user â†’ 404. GET as dataset owner â†’ 200. PATCH as viewer â†’ 403. PATCH as editor â†’ 200. Verify list endpoint silently excludes private objects from non-permitted users.
...
- [x] T036 [P] [US4] Write permission enforcement tests in `tests/test_api/test_permissions.py`: full permission matrix per data-model.md (anonymous/authenticated-no-perm/viewer/editor/owner Ã— list-public/list-private/detail-public/detail-private/create/update/delete), non-disclosure 404 for private objects, cascading permissions from dataset to sample/measurement
...
- [x] T038 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check` â€” MUST pass before proceeding
- [x] T039 âš ï¸ CRITICAL: Run User Story 4 tests: `poetry run pytest tests/test_api/test_permissions.py tests/test_api/test_filters.py -v` â€” ALL tests MUST pass

**Checkpoint â€” US4 Complete**: All permission boundaries enforced. No private data leakage. Non-disclosure behavior verified.

---

## Phase 7: User Story 5 â€” Rate-Limited Public Access (Priority: P5)
...
- [x] T041 [US5] Write rate limiting tests in `tests/test_api/test_viewsets.py`: anonymous user exceeds limit â†’ 429 with `Retry-After` header, authenticated user gets higher limit, 429 response includes descriptive message, throttle rates configurable via `REST_FRAMEWORK["DEFAULT_THROTTLE_RATES"]` override

### System Validation â€” Phase 7

- [x] T042 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check` â€” MUST pass before proceeding
- [x] T043 âš ï¸ CRITICAL: Run User Story 5 tests: `poetry run pytest tests/test_api/test_viewsets.py -v -k "throttle or rate_limit"` â€” ALL tests MUST pass

**Checkpoint â€” US5 Complete**: Rate limiting enforced for both tiers. Portal operators can configure rates.

---

## Phase 8: User Story 6 â€” Declarative API Configuration for Developers (Priority: P6)
...
**Independent Test**: Register a model with explicit `serializer_fields` â†’ verify API only exposes those fields. Register another model with a custom `serializer_class` â†’ verify API uses the custom serializer. Register a model with only `fields` â†’ verify API uses those fields as default.
...
- [x] T045 [P] [US6] Write serializer generation tests in `tests/test_api/test_serializers.py`: model with `fields` only â†’ serializer includes those fields, model with `serializer_fields` â†’ only those fields exposed, model with `serializer_class` â†’ custom serializer used directly, model with no field config â†’ default fields from model inspection, URL field present in all generated serializers, config changes reflected after app restart

### System Validation â€” Phase 8

- [x] T046 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check` â€” MUST pass before proceeding
- [x] T047 âš ï¸ CRITICAL: Run User Story 6 tests: `poetry run pytest tests/test_api/test_serializers.py -v` â€” ALL tests MUST pass

**Checkpoint â€” US6 Complete**: Registry configuration directly controls API field exposure. Three-tier resolution working.
...
- [x] T050 [P] Run quickstart.md validation: verify all code examples from `specs/011-restful-api/quickstart.md` work as documented (model registration â†’ endpoints available, token auth workflow, custom serializer override)

### System Validation â€” Final

- [x] T051 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check` â€” MUST pass
- [x] T052 âš ï¸ CRITICAL: Run full API test suite: `poetry run pytest tests/test_api/ -v` â€” ALL tests MUST pass

**Checkpoint â€” Feature Complete**: All API endpoints auto-generated, documented, permission-enforced, rate-limited, and tested. Demo app integration verified.
...
- **US5 (Phase 7)**: Depends on Phase 2 â€” can overlap with US3, US4, US6
- **US6 (Phase 8)**: Depends on Phase 2 â€” can overlap with US3, US4, US5
...
| US5 (P5) | Phase 2 validation (T013+T014) | US1â€“US4, US6 |
| US6 (P6) | Phase 2 validation (T013+T014) | US1â€“US5 |
...
- **System validation tasks** (âš ï¸ CRITICAL) MUST complete before moving to the next phase
...
T015 â†’ T016 â†’ T017 sequential (factory â†’ discovery â†’ router). T018 depends on T017 (urls include router). T019 depends on T018. T020, T021, T022 fully parallel once T019 done.
...
### Cross-Phase Parallelism (Phases 1â€“8)

US5 (Phase 7) and US6 (Phase 8) can run in parallel with US3 (Phase 5) and US4 (Phase 6) â€” they depend only on Phase 2, not on US1â€“US4.

---

## Phase 9: Base Serializer Classes â€” Implementation & Tests (FR-006 Expanded)
...
> **implemented in Phase 9 (T053â€“T055)**. `fairdm/api/viewsets.py` was cleaned up (no outstanding
...
- [x] T053 [US6] Two files require changes:
  - **`fairdm/api/serializers.py`**: Add `BaseSampleSerializer` (`Meta.model = Sample`, `Meta.fields = ["url", "uuid", "name", "local_id", "status", "dataset", "added", "modified", "polymorphic_ctype"]`) and `BaseMeasurementSerializer` (`Meta.model = Measurement`, `Meta.fields = ["url", "uuid", "name", "sample", "dataset", "added", "modified", "polymorphic_ctype"]`) â€” both are concrete `(ObjectPermissionsAssignmentMixin, serializers.ModelSerializer)` subclasses implementing `get_permissions_map()`. Add `_validate_sample_serializer(cls)` and `_validate_measurement_serializer(cls)` helpers that raise `django.core.exceptions.ImproperlyConfigured` if `cls` is not a subclass of the relevant base. Update `build_model_serializer()` signature to accept an optional `base_class` parameter (default: `serializers.ModelSerializer`) and use it in the `type(...)` call instead of the hardcoded `serializers.ModelSerializer`.
  - **`fairdm/api/viewsets.py`**: Add top-level imports for `BaseSampleSerializer`, `BaseMeasurementSerializer`, `_validate_sample_serializer`, and `_validate_measurement_serializer` from `fairdm.api.serializers` (these symbols exist in `serializers.py` after the first bullet above is done). Then in `generate_viewset()` Tier 3 path (custom `serializer_class`), re-add the validation calls after `serializer_cls = config._get_class(config.serializer_class)`: call `_validate_sample_serializer(serializer_cls)` if `issubclass(model, Sample)`, or `_validate_measurement_serializer(serializer_cls)` if `issubclass(model, Measurement)` â€” `Sample` and `Measurement` are already imported at the top of the file. In the Tier 1/2 auto-generate path, pass the correct `base_class` to `build_model_serializer()`: `BaseSampleSerializer` if `issubclass(model, Sample)`, `BaseMeasurementSerializer` if `issubclass(model, Measurement)`, else the default.
- [x] T054 [P] [US6] Write complete test coverage in `tests/test_api/test_serializers.py`: (a) field tests â€” assert `BaseSampleSerializer.Meta.fields == ["url", "uuid", "name", "local_id", "status", "dataset", "added", "modified", "polymorphic_ctype"]`; assert `BaseMeasurementSerializer.Meta.fields` matches its spec list; assert auto-generated serializer for a registered Sample subtype is `issubclass(..., BaseSampleSerializer)`; same for Measurement subtype. (b) enforcement tests â€” call `generate_viewset()` with a config whose `serializer_class` is a plain `ModelSerializer` (not inheriting `BaseSampleSerializer`) for a Sample type, assert `ImproperlyConfigured` raised; repeat for Measurement type; assert a correctly subclassed custom serializer (inheriting `BaseSampleSerializer`) succeeds without error

### System Validation â€” Phase 9

- [x] T055 âš ï¸ CRITICAL: Run serializer tests: `poetry run pytest tests/test_api/test_serializers.py -v` â€” ALL tests MUST pass before proceeding to Phase 10

**Checkpoint â€” Phase 9 Complete**: Base serializer classes and enforcement helpers implemented. Enforcement calls in `generate_viewset()` restored. Tests in place and passing. Safe to proceed with Phase 10.
...
- [x] T056 [US1] Introduce `FairDMAPIRouter(DefaultRouter)` in `fairdm/api/router.py`: subclass `DefaultRouter`, override `get_api_root_dict()` to call `super().get_api_root_dict()` then inject `ret["sample-types"] = "api-sample-discovery"` and `ret["measurement-types"] = "api-measurement-discovery"`, return `ret`; replace `fairdm_api_router = DefaultRouter()` with `fairdm_api_router = FairDMAPIRouter()` â€” no changes to URL patterns or view code required
- [x] T057 [P] [US1] Update `tests/test_api/test_router.py`: add test `test_api_root_contains_discovery_links` â€” call `GET /api/v1/` with `HTTP_ACCEPT="application/json"`, assert response status 200, assert `"sample-types"` key present in response data with a URL ending in `/api/v1/samples/`, assert `"measurement-types"` key present with a URL ending in `/api/v1/measurements/`; verify no existing `isinstance(fairdm_api_router, DefaultRouter)` assertions break (the new class still passes `isinstance`)

### System Validation â€” Phase 10

- [x] T058 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check --settings=tests.settings` â€” MUST pass
- [x] T059 âš ï¸ CRITICAL: Run router tests: `poetry run pytest tests/test_api/test_router.py -v` â€” ALL tests MUST pass

**Checkpoint â€” Phase 10 Complete**: Browsable API root lists discovery endpoints. FR-003 and FR-004 fully satisfied.
...
(lowercased, spaces â†’ hyphens) instead of CamelCase decomposition of the Python class name.
...
### System Validation â€” Phase 11

- [x] T063 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check --settings=tests.settings`
- [x] T064 âš ï¸ CRITICAL: Run router and viewset tests: `poetry run pytest tests/test_api/test_router.py tests/test_api/test_viewsets.py -v` â€” ALL tests MUST pass

**Checkpoint â€” Phase 11 Complete**: Router slugs derived from `verbose_name_plural`. All URL name assertions updated. quickstart.md documents the migration.

---

## Phase 12: User Story 7 â€” API Menu Group in Portal Sidebar (Priority: P7)

**Goal**: An "API" `MenuGroup` appears in the portal sidebar after the Measurements entry, containing
three child `MenuItem` links: "Interactive Docs" â†’ `view_name="api-docs"` (resolves to `/api/v1/docs/`), "Browse API" â†’ `view_name="api-root"` (resolves to `/api/v1/`), and
"How to use the API" â†’ `FAIRDM_API_DOCS_URL` (configurable external URL, default `"https://fairdm.org/api/"`).
...
- [x] T065 Add `FAIRDM_API_DOCS_URL = "https://fairdm.org/api/"` to `fairdm/conf/settings/api.py`; verify it is included when the API settings module is merged â€” portal developers can override it in their own settings file
- [x] ⚠️ Reopened T066 [US7] (reopened â€” BUG-001) Add "API" `MenuGroup` to `fairdm/menus/menus.py` after the `MenuCollapse` for Measurements: three child `MenuItem` entries â€” (1) `name=_("Interactive Docs"), view_name="api:api-docs", extra_context={"icon": "api"}`, (2) `name=_("Browse API"), view_name="api:api-root", extra_context={"icon": "api"}`, (3) `name=_("How to use the API"), url=getattr(django_settings, "FAIRDM_API_DOCS_URL", "https://fairdm.org/api/"), extra_context={"icon": "literature"}`; internal links use namespaced `view_name` for URL reversal â€” never hardcoded strings; add `from django.conf import settings as django_settings` import at the top of the file if not already present
- [x] ⚠️ Reopened T067 [P] [US7] (reopened â€” BUG-001) Create `tests/test_api/test_menu.py`: import `AppMenu` from `mvp.menus` and assert (1) `AppMenu` children contain a group whose `name` resolves to "API", (2) the group has exactly 3 children, (3) the first child has `view_name="api:api-docs"` and empty `_url`, (4) the second child has `view_name="api:api-root"` and empty `_url`, (5) the third child has `_url` equal to `FAIRDM_API_DOCS_URL` default (`"https://fairdm.org/api/"`) and empty `view_name`, (6) overriding `settings.FAIRDM_API_DOCS_URL` changes the Django setting value

### System Validation â€” Phase 12

- [x] T068 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check --settings=tests.settings`
- [x] T069 âš ï¸ CRITICAL: Run menu tests: `poetry run pytest tests/test_api/test_menu.py -v` â€” ALL tests MUST pass

**Checkpoint â€” Phase 12 Complete**: Sidebar "API" menu group visible on all portal pages. Three child links functional. `FAIRDM_API_DOCS_URL` setting respected.
...
- [x] T071 âš ï¸ CRITICAL: Run full API test suite: `poetry run pytest tests/test_api/ -v` â€” ALL tests MUST pass
- [x] T072 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check --settings=tests.settings` â€” MUST pass

**Checkpoint â€” Phases 9â€“12 Complete**: Base serializer tests in, API root lists discovery endpoints, router uses verbose_name_plural slugs, sidebar menu group live, developer docs updated.
...
**Breaking Change Notice**: Schema component names change globally. Clients code-generating from the OpenAPI spec (TypeScript, Python via `openapi-generator`) must regenerate. E.g., `RockSampleAPI` â†’ `RockSample`, `PatchedProjectAPI` â†’ `PatchedProject`.
...
- [x] T074 [US2] In `fairdm/api/serializers.py`, rename auto-generated serializer classes: change `build_model_serializer()` line `f"{model.__name__}APISerializer"` to `f"{model.__name__}Serializer"`; verify `_SERIALIZER_CACHE` key does not include the class name (it doesn't â€” uses `(model, fields, view_name, extra_kwargs, base_class)`) so no cache changes needed
- [x] T075 [P] [US2] Update any existing test assertions in `tests/test_api/test_serializers.py` that reference the old `*APISerializer` class naming pattern (search for `APISerializer` string); verify the serializer cache still returns the same instance for duplicate calls with identical parameters

### System Validation â€” Phase 13

- [x] T076 âš ï¸ CRITICAL: Run schema naming tests: `poetry run pytest tests/test_api/test_swagger.py -v` â€” ALL tests MUST pass
- [x] T077 âš ï¸ CRITICAL: Run full API test suite: `poetry run pytest tests/test_api/ -v` â€” ALL tests MUST pass (catch regressions from rename)

**Checkpoint â€” Phase 13 Complete**: Schema component names are clean (`RockSample`, `PatchedProject`, etc.). No "API" postfix in Swagger UI or OpenAPI schema. All existing tests still pass.
...
- [x] T080 [US2] In `fairdm/api/viewsets.py`, replace docstrings on core viewsets: `ProjectViewSet.__doc__` â†’ "Research projects registered in the portal. Projects are the top-level organizational unit containing datasets, samples, and measurements."; `DatasetViewSet.__doc__` â†’ "Datasets within research projects. Each dataset contains samples and associated measurements."; `ContributorViewSet.__doc__` â†’ "People and organizations that contribute to research projects. Contributor profiles are read-only."; `BaseViewSet.__doc__` â†’ brief developer-facing note: "Internal base class â€” see generated subclasses for API documentation."
- [x] T081 [P] [US2] Update `fairdm_demo/config.py`: ensure ALL registered model configurations have a non-empty `description` or `metadata.description` value; for models currently without descriptions, add a meaningful 1â€“2 sentence description explaining what the data type represents (this ensures the demo app demonstrates the description-flow feature)

### System Validation â€” Phase 14

- [x] T082 âš ï¸ CRITICAL: Run description tests: `poetry run pytest tests/test_api/test_swagger.py -v` â€” ALL tests MUST pass
- [x] T083 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check --settings=tests.settings` â€” MUST pass

**Checkpoint â€” Phase 14 Complete**: Swagger endpoint descriptions show meaningful model descriptions instead of internal BaseViewSet implementation details. Demo app models all have descriptions that surface in the API docs.
...
- [x] T085 [US2] In `fairdm/api/settings.py`: define `FAIRDM_API_TITLE = "FairDM Portal API"` and `FAIRDM_API_DESCRIPTION` as a rich multi-line Markdown string (covering available resources, authentication methods, rate limits, link to documentation â€” see research R17 for content); update `SPECTACULAR_SETTINGS['TITLE']` to reference `FAIRDM_API_TITLE` and `SPECTACULAR_SETTINGS['DESCRIPTION']` to reference `FAIRDM_API_DESCRIPTION`
...
### System Validation â€” Phase 15

- [x] T087 âš ï¸ CRITICAL: Run customization tests: `poetry run pytest tests/test_api/test_swagger.py -v` â€” ALL tests MUST pass

**Checkpoint â€” Phase 15 Complete**: Rich default API description and title in place. Portal developers can override via `FAIRDM_API_TITLE` / `FAIRDM_API_DESCRIPTION` settings.

---

## Final Phase (Phases 13â€“15): Verification, Documentation & Visual Inspection
...
- [x] T088 [P] Update `docs/portal-development/restful-api.md`: add section "Customizing the API Description" documenting `FAIRDM_API_TITLE` and `FAIRDM_API_DESCRIPTION` settings with example override; add section "Schema Naming Conventions" explaining that schema names match model class names without postfix; add migration note for clients code-generating from the OpenAPI schema (schema name changes from `*API` to clean names)
- [x] T089 [P] Update `specs/011-restful-api/quickstart.md`: add section "Swagger Documentation" explaining schema naming convention, endpoint descriptions from registry (`description` / `metadata.description`), and API description/title customization via settings
- [x] T090 [P] Update `fairdm_demo/config.py` docstrings: ensure each registered config class has a docstring linking to the "Developer Guide > RESTful API > Model Descriptions" documentation section, demonstrating the description-flow pattern for the demo app

### System Validation â€” Final (Phases 13â€“15)

- [x] T091 âš ï¸ CRITICAL: Run full API test suite: `poetry run pytest tests/test_api/ -v` â€” ALL tests MUST pass
- [x] T092 âš ï¸ CRITICAL: Run Django system checks: `poetry run python manage.py check --settings=tests.settings` â€” MUST pass
- [x] T093 Visually verify Swagger UI at `/api/v1/docs/` using browser: confirm (1) schema component names have no "API" postfix, (2) endpoint descriptions show model descriptions not BaseViewSet internals, (3) API title and description are rich and informative

**Checkpoint â€” Phases 13â€“15 Complete**: Schema names clean, endpoint descriptions meaningful, API description rich and customizable, developer docs updated, Swagger UI verified. Feature 011 fully complete including documentation quality improvements.

---

## Phase 16: BUG-001 â€” API URL Namespace Isolation (Spec Gap Remediation)

**Goal**: Prevent route-name collisions between API endpoints and existing server-rendered portal views by isolating API URLs under namespace `api` and updating internal reverse lookups.

**Independent Test**: Confirm `reverse("project-list")` and `reverse("dataset-list")` resolve to portal HTML routes, while API routes resolve only via namespaced names such as `reverse("api:project-list")` and `reverse("api:dataset-list")`.

- [x] T094 [US1] Update `fairdm/api/urls.py` to define `app_name = "api"` and ensure all API URL names are emitted within the `api` namespace
- [x] T095 [US1] Update API inclusion in `fairdm/conf/urls.py` to include `fairdm.api.urls` with namespace `api`, preserving existing `/api/v1/` paths while isolating names
- [x] T096 [US7] Update `fairdm/menus/menus.py` API menu links to namespaced internal view names (`api:api-docs`, `api:api-root`)
- [x] T097 [P] [US1] Add regression tests in `tests/test_api/test_router.py` and/or `tests/test_api/test_viewsets.py` asserting route-name isolation: unnamespaced `project-list`/`dataset-list` reverse to portal UI routes and namespaced API reversals resolve to API endpoints
- [x] T098 [P] [US7] Update `tests/test_api/test_menu.py` assertions to require namespaced internal API view names in menu configuration
- [x] T099 âš ï¸ CRITICAL: Run targeted regression suite: `poetry run pytest tests/test_api/test_router.py tests/test_api/test_viewsets.py tests/test_api/test_menu.py -v` â€” ALL tests MUST pass

**Bugfix**: 2026-04-13 â€” [BUG-001] Added namespacing remediation phase and reopened affected completed tasks to correct API URL-name collision behavior.
...
- **US5 (Phase 7)**: Depends on Phase 2 â€” can overlap with US3, US4, US6
- **US6 (Phase 8)**: Depends on Phase 2 â€” can overlap with US3, US4, US5
- **Polish (Original Final Phase)**: Depends on Phases 3â€“8 complete
- **Phase 9 (FR-006 impl & tests)**: Depends on Phase 8 complete â€” implemented `BaseSampleSerializer` / `BaseMeasurementSerializer` in `serializers.py` (T053â€“T055 âœ…); enforcement calls in `generate_viewset()` restored; **BLOCKS** Phases 10â€“12
- **Phase 10 (API root discovery)**: Depends on Phase 9 (T055 must pass) â€” constitution-mandated test gate
- **Phase 11 (verbose_name_plural)**: Depends on Phase 10 complete â€” URL name changes must be stable before slug refactor
- **Phase 12 (US7 sidebar)**: Depends on Phase 11 complete â€” sequential; independent of Phase 10 technically but ordered for cleanliness
- **Final Phase (Phases 9â€“12)**: Depends on Phases 9â€“12 complete
- **Phase 13 (Schema naming)**: Depends on Phases 1â€“12 complete â€” modifies `serializers.py`; must validate against full test suite
- **Phase 14 (Endpoint descriptions)**: Depends on Phase 13 (T077 must pass) â€” modifies `viewsets.py` and `config.py`; serializer rename must be stable
- **Phase 15 (API description customization)**: Depends on Phase 14 (T083 must pass) â€” modifies `settings.py` and `setup.py`
- **Final Phase (Phases 13â€“15)**: Depends on Phases 13â€“15 complete
...
| US5 (P5) | Phase 2 validation (T013+T014) | US1â€“US4, US6 |
| US6 (P6) | Phase 2 validation (T013+T014) | US1â€“US5 |
| US7 (P7) | Phase 11 complete (slug strategy stable) | â€” |

> **Note**: Phases 13â€“15 all map to US2 (Interactive API Documentation) as they improve the Swagger/OpenAPI
> documentation quality for the same set of endpoints and schemas.

### Within Each Phase

- Phase 9: T053 (implementation) â†’ T054 (tests) â†’ T055 (validation)
- Phase 10: T056 (implementation) â†’ T057 (tests) â†’ T058/T059 (validation)
- Phase 11: T060 (implementation) â†’ T061 (test updates) â†’ T062 (quickstart) â†’ T063/T064 (validation)
- Phase 12: T065 (setting) â†’ T066 (menu) â†’ T067 (tests) â†’ T068/T069 (validation)
- Final (9â€“12): T070 parallel with T071/T072; T072 after T071
- Phase 13: T073 (tests) parallel with T075 (test updates); T074 (implementation) â†’ T076/T077 (validation)
- Phase 14: T078 (tests) â†’ T079 (viewset docstrings) â†’ T080 (core viewsets) â†’ T081 (demo config) â†’ T082/T083 (validation)
- Phase 15: T084 (tests) â†’ T085 (settings) â†’ T086 (setup merge) â†’ T087 (validation)
- Final (13â€“15): T088, T089, T090 all parallel â†’ T091/T092 (validation) â†’ T093 (visual verify)
...
T053 (implementation) must complete before T054 (tests) â€” tests depend on the classes existing. T055 validation after T054.
...
T084 (tests) before T085. T085 before T086 (settings â†’ setup merge). T087 validation after T084+T085+T086.

### Final Phase (Phases 13â€“15)
...
1. MVP: Phases 1â€“3 (read-only browsing â€” highest traffic use case)
...
7. Remediate constitution violation: Phase 9 (base serializer tests â€” test-first)
8. Fix API root discovery: Phase 10 (FairDMAPIRouter)
9. Correct URL naming: Phase 11 (verbose_name_plural slug strategy)
10. Add sidebar navigation: Phase 12 (US7 â€” API menu group)
...
**Total Tasks**: 99 (T001â€“T099)
**Tasks per story**: US1=10+5=15 (incl. validation), US2=4+21=25 (incl. Phases 13â€“15), US3=6, US4=5, US5=4, US6=4+3=7 (incl. Phase 9), US7=5 (Phase 12)
**Validation checkpoints**: 33 âš ï¸ CRITICAL system validation tasks across all phases
