...
- [ ] T007 Tests in `tests/test_core/test_measurement/test_models.py` that a measurement is given a
...
- [ ] T009 Tests that a measurement requires a name, and that its own label, image, vocabulary terms
  and free-form tags are each optional.
  - **Open:** never built — no test covers the model's own field requirements
- [ ] T010 Those fields on `Measurement`, with `verbose_name`, `help_text` and translation marking.
  - **Open:** built without tests — `local_id` (`models.py:78`) has no test at all
- [ ] T011 Tests that two measurements in different datasets may carry the same label, and that both
  save.
  - **Open:** never built
...
- [X] T016 The sample relation on `Measurement`, refusing deletion while referenced.
  - **Reconciled done.** Code: `fairdm/core/measurement/models.py:71`. Test: `tests/test_core/test_measurement/test_models.py:308` — `with pytest.raises(ProtectedError): sample.delete()`
- [ ] T017 Tests that a measurement records when it was created and when it was last changed.
...
- [ ] T019 Tests that a person or organisation can be credited on a measurement under one or more
  ...
  - **Open:** never built
- [ ] T020 The contributions relation and the contributor role vocabulary binding on `Measurement`.
...
- [ ] T032 Tests in `test_admin.py` that the administrative type selection offers every registered
...
- [ ] T033 Type discovery from the registry on the parent administrative class.
...
- [ ] T034 Tests that a configuration base exists carrying the fields every measurement has, and
...
- [ ] T036 Tests in `tests/test_registry/test_config.py` that a configuration supplying an
...
- [ ] T077 Tests in `test_permissions.py` that a user holding view, change or delete over a dataset
...
- [ ] T078 The derivation of a measurement's rights from its dataset, in
...
- [ ] T079 Tests that a right granted over one measurement applies to that measurement and to no
  other.
  - **Open:** never built — skipped
- [ ] T080 Direct rights over a measurement.
  - **Open:** built without tests — skipped
- [ ] T081 Tests that rights over the sample a measurement names derive from that sample's own
  dataset, independently of the measurement's.
  - **Open:** never built — skipped
- [ ] T082 Tests that a right can be granted over a measurement of a registered type as well as
  consulted on it, and that the answers match those for the bare record.
  - **Open:** never built — skipped
- [ ] T083 Whatever normalisation that grant needs, so that a registered type is not treated as a
  record of its own.
  - **Open:** built without tests — the normalisation lives in `fairdm/core/permissions.py` and no measurement test reaches it
- [ ] T084 The backend registered in the project's authentication settings.
  - **Open:** built without tests — registered (`fairdm/conf/settings/auth.py:58`) with nothing asserting it
- [ ] T085 Confirm no test covering behaviour in this specification is skipped, and that the suite
  reports none.
...
- [ ] T098 Those inline editors on the child administrative base.
  - **Open:** built differently — same
- [ ] T099 Tests that every registered measurement type offers the same attached-record editors.
  - **Open:** never built
- [ ] T100 Tests that the administrative list names the measurement type of each row.
